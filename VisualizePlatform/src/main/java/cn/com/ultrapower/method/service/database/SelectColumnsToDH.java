...
import java.util.*;
/**
 * Created by wfz on 2018/10/10.公共查询表字段信息方法（DH库）
 */
@Service
public class SelectColumnsToDH {
    static String DB_DRIVER = "";
    static String DB_URL = "";
    static String DB_UNAME = "";
    static String DB_PWD = "";
    public SelectColumnsToDH(){
        initPath();
    }
    //查询某库某表的字段信息方法入口
    public Map executeAnalysis(Map paramMap){
        ...
        if(paramMap.containsKey("databaseName")&&paramMap.containsKey("tableName")) {
            ...
            if(StringUtils.isNotBlank(databaseName)) {
                String sql = "select name,`type` from `system`.columns WHERE database='"+databaseName+"' AND table='"+tableName+"'";
    ...
    public static void main(String [] args){
        ...
        System.out.println(new SelectColumnsToDH().executeAnalysis(testMap));
    }
}
